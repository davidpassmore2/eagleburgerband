...
export default function PortalErrorBoundary({
  ...
  return (
    <div className="p-6 sm:p-10 max-w-4xl mx-auto space-y-8">
      {/* Authenticated Member Context Header */}
      {firebaseUser && (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 text-center space-y-6 shadow-2xl">
        <div className="relative inline-block">
          <div className="w-20 h-20 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-400 mx-auto flex items-center justify-center shadow-xl shadow-rose-500/10 animate-pulse">
            <AlertTriangle className="w-10 h-10" />
          </div>
          {error.digest && (
            <span className="absolute -bottom-2 -right-2 px-2 py-0.5 rounded-full bg-rose-500 text-white font-mono text-[9px] tracking-wider shadow">
        ...
        <div className="space-y-2 max-w-lg mx-auto">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-950 border border-slate-800 text-[11px] font-mono font-bold uppercase tracking-wider text-rose-400">
            ...
            <span>Portal Feature Exception</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight">
            Portal Hit an Unscheduled Rest
          </h1>
          <p className="text-sm text-slate-400 leading-relaxed">
            An error occurred while loading this portal view. Your musician session remains securely active. You can retry the action or jump directly to another portal section.
          </p>
          {error.message && (
            <div className="text-xs font-mono text-slate-500 bg-slate-950/70 rounded-xl p-3 max-w-md mx-auto truncate border border-slate-800/80">
              Error: {error.message}
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
          <button
            type="button"
            onClick={() => reset()}
            className="inline-flex items-center gap-2 bg-yellow-400 hover:bg-yellow-300 text-slate-950 font-black px-6 py-2.5 rounded-xl uppercase tracking-wider text-xs transition shadow-lg shadow-yellow-400/10 cursor-pointer"
          >
          ...
          >
            ...
            <span>Portal Home Base</span>
          </Link>
        </div>
      </div>

      {/* Musician Portal Sections Hub */}
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <span className="text-xs font-mono font-bold uppercase tracking-wider text-yellow-400 flex items-center gap-1.5">
            ...
            <span>Portal Navigation Sections</span>
          </span>
          <span className="text-[11px] text-slate-500 font-mono">
            Jump to an active hub:
          </span>
        </div>
      ...
      <div className="text-center pt-2">
        ...
        >
          ...
          <span>Exit to Public Website (Homepage)</span>
        </Link>
      </div>
    </div>
  );
