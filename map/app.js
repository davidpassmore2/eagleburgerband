...
function formatNominatimAddress(data, fallbackLabel) {
  ...
  // 1. {title, if found}
  let title = data.name || '';
  if (title === addr.road || title === addr.house_number || title === addr.postcode) {
    title = '';
  }
  ...
  const stateVal = (addr['ISO3166-2-lvl4'] ? addr['ISO3166-2-lvl4'].split('-')[1] : null)
    || addr.state_code
    || addr.state
    || '';
...
function setLocationMarker(lat, lon, formattedData, targetZoom = null) {
  ...
  const zoomLevel = targetZoom !== null ? targetZoom : (map.getZoom() || DEFAULT_ZOOM);
  const shareUrl = buildHashUrl(lat, lon, zoomLevel);

  const addressHtml = typeof formattedData === 'object' ? formattedData.html : `<div>${formattedData}</div>`;
  const copyText = typeof formattedData === 'object' ? formattedData.plainText : formattedData;
  const inputBarText = typeof formattedData === 'object' ? formattedData.singleLine : formattedData;
  ...
  // If user clicks the pin directly, center and zoom to max
...
map.on('zoomend', () => {
  if (currentCoords) {
    const liveUrl = buildHashUrl(currentCoords.lat, currentCoords.lon, map.getZoom());
...
function navigateToCurrentLocation(lat, lon, accuracy) {
  ...
  fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&addressdetails=1`)
    ...
    .then((data) => {
      ...
      popupContent.querySelector('.copy-addr-btn').addEventListener('click', (e) => {
        navigator.clipboard.writeText(formatted.plainText).then(() => {
          e.target.innerText = '✅ Address Copied!';
          setTimeout(() => (e.target.innerText = '📄 Copy Address'), 2000);
        });
      });

      popupContent.querySelector('.copy-link-btn').addEventListener('click', (e) => {
        const liveUrl = buildHashUrl(lat, lon, map.getZoom());
        navigator.clipboard.writeText(liveUrl).then(() => {
          e.target.innerText = '✅ Link Copied!';
          setTimeout(() => (e.target.innerText = '🔗 Copy Direct Link'), 2000);
        });
      });
    ...
    .catch((err) => {
      ...
      initialMarker.bindPopup(`<strong>Your Location</strong><br>${fallback}`).openPopup();
    });
}

// 3. Zoom & Locate Controls Utility Handlers
...
// Locate Button Handler: Query GPS fresh, update markers, address, and hash
document.getElementById('locate-btn').addEventListener('click', () => {
  if (userCoordinates) {
    // If browser already located earlier, use coordinates immediately
    ...
    // Otherwise, trigger fresh geolocation prompt
...
// 4. Click-on-Map Listener: Center and Zoom to Maximum (19)
...
clearBtn.addEventListener('click', () => {
  ...
  if (userCoordinates && !initialMarker) {
    initialMarker = L.marker(userCoordinates.latlng, { icon: homeIcon })
      .addTo(map)
      .bindPopup(`Your Location (within ${Math.round(userCoordinates.accuracy)} meters)`);
    initialCircle = L.circle(userCoordinates.latlng, userCoordinates.accuracy).addTo(map);
...
if (initialSettings) {
  ...
  setLocationMarker(initialSettings.lat, initialSettings.lon, 'Looking up address...', null);

  fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${initialSettings.lat}&lon=${initialSettings.lon}&addressdetails=1`)
    .then((res) => res.json())
    .then((data) => {
      const formatted = formatNominatimAddress(data, fallback);
      setLocationMarker(initialSettings.lat, initialSettings.lon, formatted, null);
    })
    .catch((err) => {
